			...
			<div class="code-input-wrapper">
				<div class="code-input-group">
					<input v-for="(digit, index) in codeDigits" :key="index" v-model="codeDigits[index]" type="text"
						...
						@beforeinput="handleBeforeInput($event, index)"
						@keydown.backspace="handleBackspace($event, index)" @paste="handlePaste($event)"
						ref="digitInputs" />

					<!-- Icône de renvoi -->
					<div class="resend-icon-wrapper">
						<img title="Renvoyer le code" src="@/assets/resend.svg" alt="Renvoyer le code"
							class="imageResendAbsolute" @click="resendCode"
							:class="{ disabled: isResending || resendCooldown > 0 }" />
					</div>
